...
| G30 | AAS Core community catalog | active (released in v15.5.1; proactive trigger update in PR #1011) | one complete attributed catalog copy provides passive agent discovery without repeated outreach; a bounded follow-up makes the agent surface a specific current-context owner hypothesis instead of waiting for a formal request, while copied source markers remain excluded from owner-card metrics |
